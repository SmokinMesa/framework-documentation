/*
    title:  forms
    author: rob@nexuspoint.co.uk
    notes:  global, default form styling

======================================================================================================================================================================================================== */




//  FORMS - set defualts, and allow custom variables to be passed in
//  ====================================================================================================
@mixin form-styles(
    $componentName: "forms-1",

    $background-color: transparent,
    $border: 1px solid $global-border-color,
    $color: $body-color,
    $padding: .95em 1em
) {
  .#{$componentName} {
        @extend %cf;

        @for $i from 1 through $standard-cols {
            .col-#{$i} {
                @include col-($i, $col-margin: 25px);
            }
        }

        color: $color;

        //  hide the standard placeholder text
        //  ============================================================================================
        ::-webkit-input-placeholder {
            text-indent: -1000000px;
        }
        ::-moz-placeholder {
            text-indent: -1000000px;
        }
        :-ms-input-placeholder {
            text-indent: -1000000px;
        }
        :-moz-placeholder {
            text-indent: -1000000px;
        }




        //  labels and placeholders
        //  ============================================================================================
        label:not(.no-style) {
            display: block;
            height: 2rem;
            line-height: 1;
            overflow: hidden;
            pointer-events: none;
            text-overflow: ellipsis;
            transform: translateY(-1.5rem);
            transition: .3s;
            white-space: nowrap;
        }
            sup {
                float: left;
                height: 1rem;
                padding-right: .5em;
            }



        //  input, textarea, select
        //  ============================================================================================
        input:not([type="checkbox"]):not([type="radio"]),
        textarea,
        select {
            appearance: none;
            background: $background-color;
            border: none;
            border-bottom: $border;
            border-radius: 0;
            color: $body-color;
            cursor: pointer;
            height: 2rem;
            font-family: $body-font-family;
            font-size: $base-font-size;
            line-height: 1;
            margin-top: 0;
            max-width: 100%;
            transition: .3s;
            width: 100%;

            //  this class on the form element animates the label
            &.move.move {
                + label {
                    color: $color-1;
                    font-size: .714em;
                    font-weight: 600;
                    transform: translateY(.5em);    //  give it a little nudge/space
                    transition: .3s;
                }
            }


            //  add a native invalid
            &:not(:placeholder-shown):invalid {
                border-color: $color-error;
                transition: .3s;
            }


            //  add a focus brand colour
            &:focus {
                border-color: $color-1;
                transition: .3s;
            }
        }

        //  remove the select drop down from IE
        select::-ms-expand {
            display: none;
        }

        textarea {
            height: 4rem;

            &.solid {
                background-color: $body-bg-color;
                border: 1px solid $border;
                height: auto;
                min-height: 300px;
            }
        }
            .maxlength {
                font-size: .714em;
                margin: 0;
                padding: 0;
                text-align: right;
                transform: translateY(.5em);    //  give it a little nudge/space
                transform: translateY(-1.5rem);
            }




        //  icons * go AFTER label *
        //  ============================================================================================
        .icon {
            position: absolute;
            right: 0;
            // top: calc(50% - .75rem);
            top: calc(50% - 1.5em);
        }




        //  make these look like a button
        //  ============================================================================================
        select {
            &.button-style {
                @include set-icon-color(#fff);

                background: $grey-4;
                border: 1px solid $body-color;
                border-radius: 30px;
                color: #fff;
                cursor: pointer;
                display: inline-block;
                font-size: .85714rem;
                height: auto;
                line-height: 2;
                padding: .75rem 2.25rem .75rem 1.75rem; //  odd padding is for the down chevron
                text-align: center;
                width: auto;

                &.primary {
                    background: $color-1;
                    border: 1px solid $color-1;
                    color: contrast($color-1, 42%, $body-color, $body-bg-color);

                    &:hover,
                    &.hover {
                        background: mix($color-1, #000, 80%);
                    }

                    &:active,
                    &.active {
                        background: mix($color-1, #000, 70%);
                    }


                    &.outline {
                        border: 1px solid $color-1;
                        color: $color-1;

                        &:hover,
                        &.hover {
                            background: mix($color-1, #fff, 15%);
                        }

                        &:active,
                        &.active {
                            background: mix($color-1, #fff, 35%);
                        }
                    }
                }

                &.outline {
                    background-color: transparent;
                    background-image: none;
                    border: 1px solid $body-color;
                    color: $body-color;
                    position: relative;
                }

                //  set icons up
                ~ .icon {
                    @include set-icon-color(#fff);

                    position: absolute;
                    right: 1em;
                    top: calc(50% - 1.5rem);
                }

                &.primary ~ .icon {
                    @include set-icon-color(contrast($color-1, 42%, $body-color, $body-bg-color));
                }

                &.primary.outline ~ .icon {
                    @include set-icon-color(contrast($color-1, 42%, $body-color, $color-1));
                }

                &.outline ~ .icon {
                    @include set-icon-color($body-color);
                }

                label {
                    display: none;  //  select button style doesn't show label
                }
            }
        }




        //  custom checkbox/radio
        //  ============================================================================================
        input.custom {
            &[type="checkbox"],
            &[type="radio"] {
                appearance: none;
                cursor: pointer;
                height: 12px;
                position: relative;
                width: 12px;
            }

            &[type="checkbox"] {
                &::before {
                    border: 1px solid $body-color;
                    border-radius: 3px;
                    content: "";
                    height: 100%;
                    left: 0;
                    position: absolute;
                    right: 0;
                    top: 2px;
                    transition: .3s;
                    width: 100%;
                }

                &:checked {
                    &::before {
                        @include flex($align: center, $justify: center);

                        background: $color-1;
                        border-color: $color-1;
                        content: "\2714";
                        color: #fff;
                        font-size: 9px;
                        transition: .4s;
                    }
                }
            }

            &[type="radio"] {
                &::before {
                    border: 1px solid $body-color;
                    border-radius: 100%;
                    content: "";
                    height: 100%;
                    left: 0;
                    position: absolute;
                    right: 0;
                    top: 2px;
                    transition: .3s;
                    width: 100%;
                }

                &:checked {
                    &::before {
                        @include flex($align: center, $justify: center);

                        background: $color-1;
                        border-color: $color-1;
                        box-shadow: inset 0 0 0 1px #fff;
                        content: "";
                        color: #fff;
                        font-size: 9px;
                        transition: .4s;
                    }
                }
            }

            ~ label {
                cursor: pointer;
                user-select: none;
            }
        }


        //  helper type wrappers for unique layouts
        //  ============================================================================================
        //  label then radio/checkbox
        .label-input {
            @extend %cf;

            // float: left;     //  not sure whyu this floated as always in a col-
            min-height: 3rem;   //  match a "standard" input
            margin-top: 1rem;

            label {
                cursor: pointer;
                display: inline-block;
                padding-bottom: 0;

                &.disabled {
                    cursor: not-allowed;
                }
            }

            input {
                &[type="checkbox"],
                &[type="radio"] {
                    margin-left: 1em;

                    &:disabled,
                    &.disabled {
                        cursor: not-allowed;
                    }
                }
            }
        }

        //  radio/checkbox then label
        .input-label {
            @extend %cf;

            // float: left;     //  not sure whyu this floated as always in a col-
            margin-top: 1em;

            label {
                cursor: pointer;
                display: inline-block;
                padding-bottom: 0;

                &.disabled {
                    cursor: not-allowed;
                }
            }

            input {
                &[type="checkbox"],
                &[type="radio"] {
                    margin-right: 1em;

                    &:disabled,
                    &.disabled {
                        cursor: not-allowed;
                    }
                }
            }
        }


        .icon-search {
            cursor: pointer;
        }


        //  add a disabled state
        //  ============================================================================================
        :disabled,
        .disabled {
            cursor: not-allowed !important;
        }

    }   // END
}
